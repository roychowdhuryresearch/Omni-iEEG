...
class NeuralCNN(torch.nn.Module):
    def __init__(self, in_channels, outputs, freeze = False, channel_selection = True):
        ...
        self.feature_extractor = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=(3, 25), stride=(1, 25), padding=(1, 0)),  # → (B, 16, 224, 2400)
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=(3, 10), stride=(1, 10), padding=(1, 0)), # → (B, 32, 224, 240)
            nn.ReLU(),
        )
        self.in_channels = 32 # in_channels
