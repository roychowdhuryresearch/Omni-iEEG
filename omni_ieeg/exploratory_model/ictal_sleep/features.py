...
def process_negative_row(row_data):
    ...
    data, edf_channels = concate_edf([edf_name], resample=frequency)
    channels_df = filtered_dataset.get_channels_for_edf(edf_name)
    bad_channels = channels_df[channels_df['good'] == 1].name.tolist()
    missing_channels = []
    data_list = []
    channel_names = []
    labels = []
...
def extract_positive_labels(final_split, dataset_path, save_path, frequency=1000, n_jobs=8):
    ...
    training_split = final_split[(final_split['split'] == 'train')]
    
    # we then want to get patient with outcome 1 and has resection
    positive_split = training_split[(training_split['label'] == 'interictal')]
    
    # Prepare data for parallel processing
    rows_data = [(row, filtered_dataset, save_path, frequency) for _, row in positive_split.iterrows()]
...
def extract_negative_labels(final_split, dataset_path, save_path, frequency=1000, n_jobs=8):
    data_filter = DataFilter(dataset_path)
    ...
    negative_split = training_split[training_split['label'] == 'ictal']
    
    # Prepare data for parallel processing
    rows_data = [(row, filtered_dataset, save_path, frequency) for _, row in negative_split.iterrows()]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--final_split_path", type=str, default="ictal_split_cross.csv", help="Path to the ictal split file")
    parser.add_argument("--dataset_path", type=str, default="dataset/ominiieeg", help="Path to the dataset")
    parser.add_argument("--save_path", type=str, default="output/features_1min_1000hz", help="Path to save the features")
